...
class ProjectInterface(object):
    ...
    def plan_cartesian_path(self, scale=1):
        ...
        wpose.position.z -= scale * 1.375  # First move up (z)
        wpose.position.y += scale * 0.35  # and sideways (y)
        waypoints.append(copy.deepcopy(wpose))

        wpose.position.x += scale * 0.40  # Second move forward/backwards in (x)
        waypoints.append(copy.deepcopy(wpose))

        wpose.position.y -= scale * 0.20  # Third move sideways (y)
        waypoints.append(copy.deepcopy(wpose))
