...
def convert_radians_to_degrees(req):
    ...
    res.base = abs(int(((req.base + (math.pi / 2)) * 180) / math.pi))
    res.shoulder = abs(180 - int(((req.shoulder + (math.pi / 2)) * 180) / math.pi))
    res.elbow = abs(int(((req.elbow + (math.pi / 2)) * 180) / math.pi))
    res.gripper = abs(int(((-req.gripper) * 180) / (math.pi / 2)))
    return res
